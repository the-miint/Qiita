...
- `align_bowtie2_sharded(query_table, shard_directory:=, read_to_shard:=, [max_secondary, include_shard_name, preset, …])` — the bowtie2 twin of `align_minimap2_sharded`. **qiita-verified 2026-07-09** (C1 smoke + probe): same table-NAME resolution, `read_to_shard` shape, and 21-column output, EXCEPT `shard_directory` holds one SUBDIR per shard (`{shard_name}/index.*.bt2`), and — unlike minimap2 — a query batch must be UNIFORMLY single- OR paired-end: a MIX of null and non-null `sequence2` raises `gpl_boundary: … all must be non-null for paired-end`, so `align_sharded` splits SE/PE into separate calls. No `preset` needed (a bowtie2 index is preset-independent). Consumed by `align_sharded`.
