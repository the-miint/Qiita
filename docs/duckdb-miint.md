...
- SAM flag predicates: `sam_flag_*` family (paired, mapped, secondary, supplementary, duplicate, etc.) — **NOT PRESENT in the team-mirror build** (`5447847`, probed 2026-07-13): `sam_flag_secondary` etc. raise `Catalog Error: Scalar Function ... does not exist`. Use bit math on `flags` (`flags & 0x4` unmapped, `0x100` secondary, `0x800` supplementary) until the mirror carries them.
