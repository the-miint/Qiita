...
- **`@SQ` is emitted in the REVERSE of the reference-lengths table's physical order.** Build the table `ORDER BY … DESC` to land `@SQ` ascending. This matters because a BAM is "coordinate sorted" by *tid* (the `@SQ` index), not by contig name: with the table in ascending order, an `ORDER BY reference, position` produces descending tids and `jgi_summarize_bam_contig_depths` rejects the file outright (`ERROR: the bam file is not sorted!`). Verified in both directions.
- No `@HD` line is written, so there is no `SO:coordinate` tag — jgi checks the actual record order rather than the tag, and accepts a correctly-ordered file without it.
