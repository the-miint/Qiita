...
- `read_alignments(filename, [reference_lengths='table_name'], [include_filepath=false], [include_seq_qual=false])` — SAM/BAM/CRAM (alignment-shaped: `read_id`, `flags`, `reference`, `position`, …; `sequence`/`qual` when `include_seq_qual := true`). For loading a BAM as a *read source* use `read_sequences_sam` instead (below).
- `read_sequences_sam(filename)` — SAM/BAM/CRAM as READS, emitting a `read_fastx`-compatible schema. **qiita-verified 2026-07-07** (`bam_to_parquet` job + `tests/jobs/test_bam_to_parquet_miint_contract.py`; UNDOCUMENTED upstream, so this pin is the contract): columns `sequence_index BIGINT` (per-file 1-based ordinal, file order), `read_id VARCHAR`, `comment VARCHAR`, `sequence1 VARCHAR`, `sequence2 VARCHAR`, `qual1 UTINYINT[]` (phred-**decoded**, not ASCII — drops straight into the `read` table's `qual1`), `qual2 UTINYINT[]`. Emits **one row per SAM record** — it does NOT filter secondary/supplementary and exposes no FLAG column or filtering param (probed: `primary_only`/`skip_secondary`/… all `BinderException`), so a paired mate or secondary/supplementary alignment appears as a repeated `read_id` (bam_to_parquet rejects those via a `count(DISTINCT read_id)` guard). There is no FLAG column, so it cannot tell aligned from unaligned records — bam_to_parquet therefore requires the caller to declare unaligned input rather than verifying it. A headerless SAM (no `@SQ`) is rejected by htslib. A header-only SAM yields **zero rows** (not a throw — unlike `read_fastx`). **Verified:** SEQ is returned verbatim (reference-forward per SAM for reverse-strand records, not reverse-complemented to sequenced orientation) — which is why bam_to_parquet requires unaligned input.
