...
>
> So: for "does X support Y?", the authorities are the rendered docs above, the upstream `.md`, or the source — **not** an experiment. Two habits that would have caught it:
> - **A `BinderException` naming one parameter is proof there is a parameter list.** `COPY FORMAT BAM requires REFERENCE_LENGTHS parameter` should prompt "what else is on this writer?", not just "add that one argument."
> - **Before writing "miint cannot …" anywhere** — comment, changelog, issue, or chat — open the upstream page for that function. Absence claims are the ones that get copied forward unchallenged, because nobody re-probes a documented dead end.
...
| --- | --- | --- | --- |
| [duckdb-miint#173](https://github.com/the-miint/duckdb-miint/issues/173) | `COPY … (FORMAT SAM\|BAM)` emits `@SQ` in an order **not derivable** from the `REFERENCE_LENGTHS` table (nor its reverse, nor `ORDER BY reference`), and the writer takes no sort parameter — so a coordinate-sorted BAM cannot be produced from SQL. Deterministic, but the rule is unknown. See the `FORMAT BAM` writer entry below. | `workflows/long-read-assembly/binning.sh` runs `samtools sort` on the coverage BAM before staging it for metaWRAP (~13.5 s + a second reads-sized artifact per ticket); `assembly_coverage` deliberately applies **no** `ORDER BY`. Canary: `test_sq_order_is_not_derivable_from_reflen` fails if `@SQ` ever gains a defined order. | [the-miint/Qiita#374](https://github.com/the-miint/Qiita/issues/374) |
| [duckdb-miint#174](https://github.com/the-miint/duckdb-miint/issues/174) | No SQL access to a SAM/BAM **header**: `read_alignments` / `read_sam` return per-record columns, and `SELECT DISTINCT reference` is not a substitute — it drops zero-coverage references, which are exactly the ones an `@SQ` assertion is about. | `_sq_reference_names` in `qiita-compute-orchestrator/tests/jobs/test_assembly_coverage.py` hand-parses the BAM binary header (magic → `l_text` → `n_ref` → per-ref name/length). Test-only; nothing in the job reads a header. | No Qiita issue — replace that helper with the reader when one lands. |
...
- **`@SQ` ORDER IS NOT CONTROLLABLE FROM THE REFERENCE-LENGTHS TABLE — do not build a sort strategy on it. qiita-probed 2026-07-24** (`open_miint_conn`, build `v1.5.4`; the same result was reproduced standalone on the deploy host). This entry previously asserted that `@SQ` comes out in the *reverse* of the reference-lengths table's physical order, so that building the table `ORDER BY … DESC` lands `@SQ` ascending. **That is false**, and it cost a production ticket (`long-read-assembly` `binning` died in `jgi_summarize_bam_contig_depths` 2.15 with `ERROR: the bam file 'reads.bam' is not sorted!`). What the probe actually shows:
  - The emitted `@SQ` order equals **neither** the reflen table's physical order, **nor** its reverse, **nor** `ORDER BY reference` — at n = 5, 10, 64, 300, 2000, and whether the table is built ASC, DESC, or shuffled. The **set** of names is always preserved; only the order is unpredictable.
  - The reversal appears to hold only at tiny n and depends on the names: with `s1.ctg%06dl`-style contigs a DESC table yields ascending `@SQ` up to n=5 and diverges from n=6; with `s<i>.ctg%06dl` names it already diverges at n=3.
  - It **is deterministic** — byte-identical `@SQ` order across repeated runs on fresh connections (n=300, 3 runs), and independent of thread count (1/4/8, checked on the deploy host).
  - **The rule itself is UNKNOWN.** miint's source was not read; do not infer one from the samples above.
  - **Filed upstream as [duckdb-miint#173](https://github.com/the-miint/duckdb-miint/issues/173)** (a defined or steerable `@SQ` order), with the workaround's removal tracked at [the-miint/Qiita#374](https://github.com/the-miint/Qiita/issues/374) — see [Open upstream gaps](#open-upstream-gaps). Reading `@SQ` back to check any of this needs a hand-parse of the BAM header, since miint exposes no header reader: [duckdb-miint#174](https://github.com/the-miint/duckdb-miint/issues/174).
  - **There is no parameter for it** — a *capability* answer, taken from the rendered upstream docs as this file's own rule demands, not from an experiment. The documented `FORMAT SAM`/`FORMAT BAM` parameter list is exactly `INCLUDE_HEADER`, `REFERENCE_LENGTHS`, `SEQUENCE_DATA`, `COMPRESSION` (SAM only) and `COMPRESSION_LEVEL` (BAM only); the words `@SQ`, `sort`, `sorted` and `SO:coordinate` do not appear in that section at all, and the emitted `@SQ` order is not described. So there is nothing to pass to get a defined order — a `samtools sort` downstream is the only lever. (Upstream also documents that multithreaded BGZF compression "preserves record order and the output is byte-identical to single-threaded writing", which is consistent with the determinism observed above.)
  - Consequence: a BAM is "coordinate sorted" by *tid* (the `@SQ` index), not by contig name, so `ORDER BY reference, position` in the `COPY` is **not** a coordinate sort. If anything downstream requires a sorted BAM, run `samtools sort` on the writer's output — which is what `workflows/long-read-assembly/binning.sh` now does before handing the BAM to metaWRAP.
- No `@HD` line is written by miint's `FORMAT BAM` writer, so its output carries no `SO:coordinate` tag. (jgi checks the actual record order rather than the tag, so the missing tag is not itself the problem — the record/tid order is.) A `samtools sort`ed BAM **does** carry `@HD VN:1.6 SO:coordinate`, verified — so the header gains the tag once the file passes through the sort above.
