...
Migration `20260527000000_bump_identity_start_to_25k.sql` fast-forwards the implicit identity sequences for `qiita.study.idx` and `qiita.prep_sample.idx` past the reserved block (`ALTER TABLE … ALTER COLUMN idx RESTART WITH 25000`). The bump is one-way: `RESTART` advances a sequence but cannot rewind it without risking PK collisions, so the migration's down step is a deliberate no-op.
