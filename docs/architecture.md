...
The current `host-reference-add` builders still read chunks from **staging** (they run before `register-files`; see below); migrating them onto this streaming path (and the per-shard alignment-subject builders that consume it) is a later milestone.
