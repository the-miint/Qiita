...
| `assembly_membership` / `bin_quality` | `(prep_sample_idx, processing_idx)` | Which features a `(prep_sample, run)` assembly contains and in which bin, and per-MAG CheckM quality. The DuckLake copies, for bulk joins against the sequences; **sinks** — absent from `ALLOWED_TABLES`, so neither is nameable by a ticket. `assembly_membership` is read by the data plane as the assembly surfaces' scope resolver (above), never streamed |
