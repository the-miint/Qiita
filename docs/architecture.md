...
This `PATH_DERIVED` tree is **derived storage**, the orchestrator's third storage concern alongside the data plane's persistent DuckLake data and the orchestrator's own ephemeral per-attempt workspace. It is durable but per-reference and compute-side: the two builder jobs write it, `host_filter` reads it (via the DB-recorded `reference_index.fs_path`), and `DELETE /reference-artifact/{idx}` purges the whole `references/{idx}` subtree. The `{PATH_DERIVED}/references/{idx}/...` layout has one owner — `qiita_compute_orchestrator/derived_store.py` (`reference_derived_dir` / `rype_index_path` / `rype_router_index_path` / `minimap2_index_path` / `bowtie2_index_path` / `reference_shard_dir` / `shard_rype_index_path` / `shard_minimap2_dir` / `shard_minimap2_index_path` / `shard_bowtie2_dir` / `shard_bowtie2_index_prefix`) — which all consumers call rather than rebuilding the path by hand. **C1 pinned the per-shard aligner layout to the exact `shard_directory` shape miint's `align_{minimap2,bowtie2}_sharded` expects** (verified against the real build — see [duckdb-miint.md](duckdb-miint.md)): each aligner has ONE per-reference shard-directory root that `align_sharded` is pointed at, holding one entry per shard — minimap2 a FLAT `references/{idx}/minimap2-shards/{shard_id}.mmi` (`shard_minimap2_dir` / `shard_minimap2_index_path`), bowtie2 a per-shard SUBDIR `references/{idx}/bowtie2-shards/{shard_id}/index*` (the multi-file `.bt2` set; `shard_bowtie2_dir` / `shard_bowtie2_index_prefix`). The C1 read router is a single whole-reference multi-bucket rype `.ryxdi` at `references/{idx}/rype-router.ryxdi` (`rype_router_index_path`). The per-shard rype `.ryxdi` at `references/{idx}/shards/{shard_id}/index.ryxdi` (`shard_rype_index_path`, B5 fan-out) is now **vestigial for routing** — C1 routes with the whole-reference router; C2 removes the per-shard rype build. All of these stay under `references/{idx}`, so the same single-subtree `DELETE /reference-artifact/{idx}` purge covers them. Because these paths sit outside `$QIITA_OUTPUT_PATH`, a builder must never declare its index as a step output (it would fail gate 4); the location travels in the step's in-tree meta JSON, which `register-index` reads. See the native-step note under "Container contract".
...
All three per-shard build jobs consume this streaming path: `build_rype_index` (the routing `.ryxdi`) plus the alignment-subject `build_minimap2_index` / `build_bowtie2_index`, each in shard mode. Their small `feature_idx` roster rides the ticket and the chunk bytes stream from DuckLake. The minimap2/bowtie2 builds reassemble a `(read_id, sequence1)` subject (`subject.stage_subject`, single-sourced with the host reassembly) that miint's `save_{minimap2,bowtie2}_index` indexes; the rype build registers the streamed `(feature_idx, chunk_index, chunk_data)` relation directly as the chunk table for `rype_index_create`. The ticket-fetch + stream composition is `data_plane_client.open_reference_chunk_stream`. B5 folded the rype shard build onto this path (a shard build runs after the ingest ticket's `register-files` has moved the staging chunks into DuckLake, so there is no staging Parquet left to read). The whole-reference **host** builders still read chunks from **staging** (they run before `register-files`; see below), byte-identical to before; migrating those onto streaming is a later milestone.

#### Sharded-index fan-out (B5)

B5 turns the planner + per-shard builders into an **end-to-end sharded build for a whole analysis reference**: assign features to shards, fan out one build ticket per shard, and gate the reference `active` on all shards completing. It is **opt-in** — `reference-add` / `local-reference-add` carry a `shard_index` context flag (plus `build_rype` / `build_minimap2` / `build_bowtie2` gates and `rype_w` / `minimap2_preset` knobs); default (flag absent) is byte-identical to today's `loading → active` with no build. Sharding is an indexing choice *on* a reference, not a separate action — the same reference data exist whether it is sharded or not.
...
- **Fan-out** (`shard_orchestration.plan_and_submit_shards`): when N>0 it transitions the reference `loading → indexing` and, in one transaction, INSERTs one PENDING `build-shard-index` work ticket per shard (scope `reference`, `shard_id=k`, the index-selection context copied from the parent), then dispatches each. When N=0 it is a no-op (the parent finalize applies `active`). This is deliberately **lighter than the block stack**: a shard is a clean partition of one reference (the reference is the accounting unit), so there is no `block` / `block_member` cover-map, no gate table, and no new `scope_target_kind` — the cover-map is `reference_membership.shard_id` and every ticket stays `reference`-scoped, discriminated only by the new `work_ticket.shard_id` column. That column also re-partitions the `work_ticket_one_in_flight_per_reference` unique index (now `… AND shard_id IS NULL`) and adds `work_ticket_one_in_flight_per_shard`, so N same-action shard tickets for one reference no longer collide while the one-per-reference guarantee holds for every unsharded action. Idempotent on redrive (`ON CONFLICT DO NOTHING`; a crash between INSERT and dispatch leaves tickets PENDING for the startup reconcile).
- **`build-shard-index/1.0.0`** is the per-shard workflow each ticket runs (`target_kind: reference`). The runner stages the shard's roster before the step loop (`_stage_shard_roster`: a `feature_idx`-scoped `reference_sequences` DoGet for this `shard_id`'s members → `shard_roster.parquet`, binding `shard_features` + `shard_id`), then runs the gated `build_{rype,minimap2,bowtie2}_index` steps — each with a `register-index` sibling that routes `meta.shard_id` to a per-shard row — and ends with `finalize-shard`. The workflow carries **no** `success_status` / `failure_status`: `finalize-shard` owns `indexing → active`, and a `failure_status` would wrongly flip the whole reference to `failed` on a single shard's failure.
- **`finalize-shard`** (`actions.library.finalize_shard`, the terminal step of each shard's ticket) is **count-based and fail-closed**. It derives N from `reference_membership` (COUNT DISTINCT `shard_id` — no `shard_count` column to keep in sync), counts registered shards per expected `index_type`, and does the guarded `indexing → active` only when every expected type reaches N (and at least one type and one shard exist, so a degenerate empty expected-set can never vacuously activate). A still-missing shard leaves the reference honestly in `indexing`; it **never** transitions to `failed` (the FSM's only exit from `failed` is `→ pending`, a full-ingest restart — wrong blast radius). Race-safe: register rows are dedup'd and committed before each ticket's own finalize, so the last finalize in wall-clock time observes every sibling's rows and the guarded UPDATE lets exactly one racer flip `active` (a finalize that finds the reference already `active` treats the illegal-transition as idempotent success).
- **Conditional finalize** (runner): the parent reference-add's `success_status: active` patch is skipped while a sharded fan-out is in progress (the reference is `indexing` and `shard_index` is set), leaving `active` to `finalize-shard`. Unsharded / N=0 / host paths patch `active` inline, unchanged.
...
#### Sharded alignment (C1)

B5 *produces* the per-shard indexes; **C1 is the consuming side** — align a block of reads against a sharded reference by aligning each read against only the shard(s) it minimises into, not the whole backbone. The C1 milestone lands three native jobs and the miint contract they rest on; it is **native-job-only and not yet wired into a workflow** (the runner block × shard fan-out, the DuckLake alignment-detail sink, and `register-files` are C2).

- **Routing.** Rather than one `rype_classify` pass per shard (N passes, impractical at `_SHARD_COUNT=1000`), C1 uses a single **whole-reference multi-bucket rype router**: `build_routing_index` streams the whole reference's chunks from the data plane and runs `rype_index_create` with a `shard_mapping` `(feature_idx, bucket_name = str(shard_id))` — one bucket per shard — writing `references/{idx}/rype-router.ryxdi` (`rype_router_index_path`). One `rype_classify` pass against it emits `(read_id, bucket_name)` = the `read_to_shard` table (multi-bucket: a read whose minimisers span K shards yields K rows). The B5 per-shard rype `.ryxdi` is now vestigial for routing; C2 removes it. (Router construction can UNION over multiple routers later, so new shards can land in a separate router without a full rebuild — one router is built now.)
- **Alignment.** `align_sharded` (modelled on `host_filter`) builds a query VIEW `(read_id = sequence_idx, sequence1, sequence2)` over the staged read block, builds `read_to_shard` via one router `rype_classify`, then calls `align_{minimap2,bowtie2}_sharded(query, shard_directory:=, read_to_shard:=)` pointed at the aligner's per-shard root (`minimap2-shards` flat `.mmi`s, or `bowtie2-shards` per-shard subdirs). The aligner output's `reference` is the subject's stored id (our `feature_idx`), so `feature_idx = CAST(reference AS BIGINT)`; the job joins `prep_sample_idx` per row and streams a sorted `alignment.parquet`. Single-end and paired-end reads are aligned as **separate uniform sub-batches** because `align_bowtie2_sharded` rejects a query mixing null/non-null `sequence2`.
- **No dedup.** Every alignment row is emitted; `(sequence_idx, feature_idx)` is deliberately **not** unique. Multiplicity has two intentional sources: cross-shard (a read routed to K shards → K rows with DISTINCT `feature_idx`, since a feature belongs to exactly one shard) and per-mate (a PE read aligning in one shard → one SAM row per mate, same `feature_idx`, different `flags`/`position`). The alignment output carries `feature_idx` but **not** `reference_idx` — reference scoping is a query-time join against `reference_membership` (see "Identifier ownership").
- **Deferred to C2.** The shard-aware index resolver (mapping `reference_idx` → the router path + per-aligner `shard_directory` from `reference_index` rows) is not built in C1: the control plane resolves index paths from DB `fs_path` rows, and no router/shard rows exist until C2 registers them, so C1's smoke passes the paths directly. How a block-scoped `align_sharded` ticket carries `reference_idx` (likely via workflow `params:`, like `host_filter`'s `mask_idx`) is also a C2 decision. C1 registers **no** `reference_index` row for the router, so the `index_type` CHECK gains `rype_router` only when C2 starts inserting it.
