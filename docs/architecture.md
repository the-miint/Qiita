...
| `assembled_sequence` / `assembled_sequence_chunks` | `feature_idx` | One assembly run's contigs, content-hash deduped into the shared feature space exactly like a reference sequence: `(feature_idx, sequence_hash, sequence_length_bp)` plus the bytes in 64 KB chunks. Both are in `ALLOWED_TABLES`, scoped by `feature_idx` — neither carries `prep_sample_idx`, so the control plane resolves a run's roster from `qiita.assembly_membership` and signs it (`POST /assembly/ticket/doget`) |
| `assembly_membership` / `bin_quality` | `(prep_sample_idx, processing_idx)` | Which features a `(prep_sample, run)` assembly contains and in which bin, and per-MAG CheckM quality. The DuckLake copies, for bulk joins against the sequences; **sinks** — absent from `ALLOWED_TABLES` |
