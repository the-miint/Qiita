...
- **Deferred to C2b.** The `align` workflow, block × shard fan-out / submit route, the DuckLake `alignment` sink + `register-files` of alignment rows, and alignment-as-processing (`processing_idx`, disallow-without-delete, completion accounting). How a block-scoped `align_sharded` ticket carries `reference_idx` (likely via workflow `params:`, like `host_filter`'s `mask_idx`) is a C2b decision. A **forward constraint** shapes it: the user wants a reference collection to *grow* — add shards over time and align prior reads against only the NEW shards — which leans toward per-shard processing identity (not one `processing_idx` over the whole reference) and append-only shard growth (the current fixed-count clear-first planner re-tiles on growth; a growable-reference design must make shards append-only). The resolver's LIST-of-routers return anticipates this.
