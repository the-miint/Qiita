...
| `alignment` (C2b) | `(alignment_idx, prep_sample_idx, sequence_idx, feature_idx, position)` | Per-read sharded-alignment results, keyed by the mask-style `alignment_idx` (NOT `processing_idx` — that hierarchy is deferred): 5 CP identity columns (`alignment_idx, prep_sample_idx, sequence_idx, feature_idx, mate_feature_idx`) + the verbatim miint aligner output (`flags, reference, position, …, tag_sa`). Created by `ensure_alignment_tables`; a **sink** — not in `ALLOWED_TABLES` (no Flight read-side yet) |
...
The **`alignment`** table is created by the data plane (`ensure_alignment_tables`, C2b) and is keyed by `alignment_idx` — the formal `processing_idx` / `processed_prep_sample_idx` hierarchy remains deferred (see "C2b: alignment consumer"). The **Count / aggregation** row still describes a planned processing-results layout not yet created by the data plane; `ensure_read_tables`/`ensure_reference_tables`/`ensure_alignment_tables` create the reference, read, and alignment tables today.
...
Read **routing** is served by ONE whole-reference multi-bucket rype `.ryxdi` (the `rype_router`, built by `build_routing_index` — see "Sharded alignment" below), not a per-shard `.ryxdi`: C2 removed the per-shard rype build, so `build_rype_index` is now host/whole-reference only. The per-shard **alignment-subject** indexes (minimap2 `.mmi` + bowtie2 `.bt2`) are built by `build_minimap2_index` / `build_bowtie2_index` in shard mode (B4; see "Reference sequence streaming" below): given a `shard_id` and a runner-staged feature roster (`shard_features`, a Parquet of `(feature_idx, sequence_length_bp)`), each streams that shard's chunks and writes its per-shard index at the `minimap2-shards/{shard_id}.mmi` / `bowtie2-shards/{shard_id}/index*` paths above, recording `shard_id` in the meta JSON `register-index` records. B5 lands the orchestration that turns the tiler into an end-to-end sharded build — feeding real lineages compute-side, the fan-out over shards, and count-based completion — see "Sharded-index fan-out" below. **C1 consumes these indexes** — see "Sharded alignment" below.
...
- **`build-shard-index/1.0.0`** is the per-shard workflow each ticket runs (`target_kind: reference`). The runner stages the shard's roster before the step loop (`_stage_shard_roster`: a `feature_idx`-scoped `reference_sequences` DoGet for this `shard_id`'s members → `shard_roster.parquet`, binding `shard_features` + `shard_id`), then runs the gated `build_{minimap2,bowtie2}_index` steps — each with a `register-index` sibling that routes `meta.shard_id` to a per-shard row — and ends with `finalize-shard`. (Per-shard rype is gone — routing is the whole-reference `rype_router` the PARENT reference-add builds; C2.) The workflow carries **no** `success_status` / `failure_status`: `finalize-shard` owns `indexing → active`, and a `failure_status` would wrongly flip the whole reference to `failed` on a single shard's failure.
...
- **Fan-out.** `align_planner.plan_and_submit_alignments` (mirroring `block_planner`) enumerates the pool's samples, LOOKS UP each sample's already-minted `mask_idx` (never mints — a sample with no minted/`completed` mask is skipped), asserts the reference is ACTIVE + sharded via `_resolve_sharded_align_indexes`, partitions the to-align samples by `mask_idx`, mints one `alignment_idx` per partition, tiles into blocks (reusing `tile_partition`), and creates one block `work_ticket` per block carrying `mask_idx` + `alignment_idx` + an `action_context` of `{align_reference_idx, aligner, alignment_idx, align_mask_idx}`. The submit route is `POST …/sequenced-pool/{P}/align-plan` (`prep_sample:write` + `wet_lab_admin`). The runner resolves `router_index_path` (= `router_paths[0]`) + `shard_directory` from `action_context` (the C2a resolver) before the loop and threads `aligner` + `alignment_idx` into `align_sharded` via `params:`.
