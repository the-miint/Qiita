# Changelog archive — rotated 2026-08-24 at eeb3bd0c

305 entries covering PRs #29–#386, moved out of `CHANGELOG.md` when that file
reached 5,745 lines. They are the duplicate `### Added` / `### Fixed` /
`### Changed` / `### Removed` bucket headings that accumulated *below* the four
headings PRs actually write to — eleven redundant buckets in all.

PR-ref ranges overlap with the live file (#161–#472). The strata are not in
chronological order: each wave of PRs added a fresh bucket heading at the top
rather than scrolling to the existing one, so older and newer entries interleave
between strata.
