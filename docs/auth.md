...
`alignment:doget` (mint a Flight DoGet ticket for an alignment cohort you name yourself) is on **every** role ceiling, unlike the worker scopes above, and is deliberately **absent** from `SERVICE_ACCOUNT_SCOPE_CEILING` — the mirror image of `ticket:doget`. On the alignment surface the two carry the same data and different trust models (`ticket:doget` additionally opens surfaces this one never reaches — `Scope.TICKET_DOGET` enumerates them): `ticket:doget` signs a cohort the control plane read out of a work ticket, which the runner resolver validated at submit; `alignment:doget`'s route has no such upstream, so it authorizes the caller's cohort per-study (`Tier.VIEWER` on every study each sample links to) before signing. Because that per-study check is the real boundary *for a plain user*, the role does not need to be one — hence every ceiling. Note the limit of that argument: `wet_lab_admin` and `system_admin` sit at or above the cohort gate's bypass role, so their cohort is returned whole with no per-study lookup and the 403 cannot fire. For those two the role *is* the boundary. Splitting them is what keeps a human PAT off the worker path and a worker off the human one, so a single data-plane surface never has two validation paths.

`assembly:doget` (mint a Flight DoGet ticket for ONE assembly run's contigs, naming the run yourself) rests on `alignment:doget`'s argument unchanged, and lands in the same place: on **every** role ceiling and absent from `SERVICE_ACCOUNT_SCOPE_CEILING`. It widens *who* may ask for an assembly's contigs — the two sequence surfaces (`assembled_sequence` / `assembled_sequence_chunks`) and the signed filter are the service route's, and `Scope.TICKET_DOGET` already classes an assembled contig as not raw human/host reads. It also signs one surface the service route does not: `bin_quality`, the run's per-subject CheckM rows, whose privacy argument is on the data plane's `ALLOWED_TABLES` entry. The one thing specific to it: its route checks per-study access **before** existence, inverting the alignment mint's ladder, because the thing that may not exist here is `(this prep_sample, this run)` — answering 404 first would tell a caller whether a prep_sample they cannot read was assembled. The run's `feature_idx → genome_idx` map beside it is not a ticket at all and sits at `prep_sample:read`: it returns identifiers and provenance, no sequence.
...
| `/api/v1/admin/study/{idx}/owner-biosample-id` | GET | Re-identification export: maps the study's `biosample_idx` + `biosample_accession` back to the owner-submitted original name (the PII-pinned `biosample_metadata` value flagged `is_owner_biosample_id`, kept off the general `biosample:read` paths and returned elsewhere only by the admin-clamped study-scoped biosample read surface). Optional `?sequenced_pool_idx=` restricts to that pool's `sequenced_sample`s in the study and adds `prep_sample_idx` + ENA experiment/run accessions. 404 on unknown study or pool. Requires `admin:biosample_owner_id_read`. |
...
| `/api/v1/admin/masked-read-export/ticket` | POST | Mints a Flight DoGet ticket scoped to one `(prep_sample_idx, mask_idx)` on the data plane's `read_masked` macro (the broadly-reachable Flight read surface; the members-scoped `read_block` selector reaches raw `read` rows for one block, gated on `read:doget`; an unconditional `reason='pass'` redacts host/QC reads). Body `{prep_sample_idx, mask_idx}`, both `gt=0`; the signed filter is re-asserted non-empty before signing — defence in depth behind the macro's own required arguments. The human (`system_admin`) counterpart to the service-account `POST /read-masked/ticket/doget`. Minted at the data plane's 3600 s max (expiry checked only at DoGet initiation, so it never bounds the download). Requires `admin:masked_read_export`. |
...
| `/api/v1/reference/exclusion` | DELETE | Unblock by `?genome_idx=` XOR `?feature_idx=` query param — a **soft delete**: the row is kept and stamped `unblocked_at`/`unblocked_by_idx` (preserving the curatorial record), never hard-deleted. Idempotent (`changed=false` if nothing is actively blocked). Re-syncs the mirror. Requires `reference:exclusion:write` + a complete profile (the actor becomes `unblocked_by_idx`). Registered *before* `DELETE /reference/{reference_idx}` so the literal `exclusion` path isn't shadowed by the int-param route. |
...
| `/api/v1/reference/{reference_idx}/genome-map/parquet` | GET | The same rows as the JSON route above, as a Parquet body (`application/vnd.apache.parquet`), with **no cap** — the projection is shared SQL text, so the two forms cannot disagree. This is the form the client-side `feature-table build` recipe reads, the JSON route's cap being below what the deploy's references need. Completeness needs no flag because a truncated Parquet body fails to parse. Same `404` / empty behaviour, same `reference:read`; no new scope. |

### Assembly

The de novo arm: two DoGet mints over one assembly run's contigs, the run-scoped
`feature_idx` → `genome_idx` map and membership rows beside them, and the roster an
export walks. Why `assembly:doget` sits where it does, and why the map is not a ticket
at all, is in the Scopes and roles section above.

| Route | Method | Notes |
|---|---|---|
| `/api/v1/assembly/ticket/doget` | POST | Sign a DoGet ticket scoped to one assembly run, minted at job **runtime** (a SLURM queue can outlive a submit-time ticket). Service-account only, on the generic `ticket:doget` — that scope is on `SERVICE_ACCOUNT_SCOPE_CEILING` and on no human role ceiling, so a human PAT cannot carry it. The signed filter is the `(prep_sample_idx, processing_idx)` pair and nothing else: `extra="forbid"` leaves no body field that reaches the filter, and the data plane refuses a `feature_idx` on the two assembly surfaces outright, so neither end of the wire can widen or narrow a run. `422` for a table outside `assembled_sequence` / `assembled_sequence_chunks`; `404` and never a ticket for a pair that never assembled — the same answer as an unknown `prep_sample_idx` or `processing_idx`, because there are no contigs to stream either way. Authorization is scope-only at this layer, matching the reference / read_masked / alignment mints. |
| `/api/v1/assembly/{prep_sample_idx}/{processing_idx}/ticket/doget` | POST | The scientist-facing counterpart, where the caller names the run. Signs the two contig surfaces and also `bin_quality`, the run's per-subject CheckM rows, which the service route does not. `assembly:doget` + a complete profile, and `Tier.VIEWER` on every study the run's prep_sample is still linked to. **Access is checked BEFORE existence**, inverting the alignment mint's ladder: the thing that may not exist here is `(this prep_sample, this run)`, so answering `404` first would tell an unauthorized caller whether a prep_sample they cannot read was assembled. The signed run is the authorization boundary either way — the data plane serves exactly the pair the ticket carries and knows nothing about studies or users. Run state gates all three run-scoped routes alike: `404` for a run with no row or one closed at `no_data` (no contigs to serve), `409` for `pending` or `invalidated`. `pending` means the row never closed, which covers a run still going *and* one whose ticket failed or was cancelled — nothing sweeps the gate, so a dead run reads `pending` until a resubmission under the same params re-resolves it; `invalidated` was judged untrustworthy by a person. |
| `/api/v1/assembly/{prep_sample_idx}/{processing_idx}/genome-map` | GET | One run's contig → `(genome_idx, source, source_id)` lookup, ordered by `(feature_idx, genome_idx)` — the de novo twin of `/reference/{idx}/genome-map`, and a control-plane read for the same reason: `genome_idx` and a genome's provenance exist only in Postgres. Requires `prep_sample:read` rather than the DoGet scope beside it, because this returns identifiers and provenance, no sequence and no ticket. Access is per-study and checked first, and the same `404` / `409` run-state gate as the mint above applies. **`422`, not a short `200`, when any MAG or LCG membership of the run carries no `genome_idx`** (an UNBINNED contig has no genome to reach the map with, so it is not counted — refusing over it would refuse a gap that cannot affect the answer) — those contigs are simply absent from the map, and the absence is invisible downstream: the genomes they belong to keep their other contigs, so length denominators come back short and breadth comes back high. The refusal names the backfill that fixes it. `413` above the hard cap, naming the real size, and no `truncated` field. |
| `/api/v1/assembly/{prep_sample_idx}/{processing_idx}/genome-map/parquet` | GET | The same rows as Parquet (`application/vnd.apache.parquet`) with **no cap**, through the identical gates — the `422` on unminted memberships still fires, and still fires before any body exists. A zero-row run is a `200` carrying a valid zero-row Parquet — a completed run whose contigs are all UNBINNED — which degrades to the reference arm rather than erroring; a run that produced no contig at all closes at `no_data` and is `404` at the gate, never a zero-row body. Same `prep_sample:read` and the same `404` / `409` run-state gate; no new scope. |
| `/api/v1/assembly/{prep_sample_idx}/{processing_idx}/membership` | GET | Every `qiita.assembly_membership` row of one run — `feature_idx`, `kind`, `bin_id` and the assembler's `raw_name` / `circularity` / `depth` / `mult` — every kind including UNBINNED, ordered by `(kind, bin_id, feature_idx)`. The genome map's gates without its `422`: no `genome_idx` is returned, so an unminted row is not a gap here. `413` above its own cap, sized to the genome map's body budget. The Postgres copy, which keeps a superseded row where the lake replaced it, so `qiita assembly export` compares the contig set these rows name with the one the data plane streams and refuses when they differ (a contig moved between subjects by a re-run is not detected). Requires `prep_sample:read`. |
| `/api/v1/assembly/{prep_sample_idx}/{processing_idx}/membership/parquet` | GET | The same rows as Parquet with no cap, through the same gates. |
| `/api/v1/assembly/{processing_idx}/prep-sample` | GET | The prep_samples gated under one run that the caller may **read** (`Tier.VIEWER` on every linked study, or wet_lab_admin+), each with its gate state and biosample accession; optional `sequenced_pool_idx`, `study_idx` or `prep_sample_idx`. `/processing/{idx}/prep-sample` narrows a plain user at `Tier.ADMIN`, the submit tier, so a viewer gets none of these prep_samples there. Narrowed for a pool or study; a named `prep_sample_idx` the caller cannot read is a `403` before the run is looked up. `404` for an unknown run, `413` rather than a truncated list. Requires `prep_sample:read`. |

### Read mask

| Route | Method | Notes |
|---|---|---|
| `/api/v1/mask-definition/{mask_idx}/syndna-read-count` | GET | Per-insert SynDNA read counts under one mask for the prep_samples selected by `study_idx`, `sequenced_pool_idx` and/or repeatable `prep_sample_idx` (intersected; at least one required, else `422`): the reference's inserts with their recorded accession, and per prep_sample its biosample accession, `sequenced_pool_idx` and one count per insert. All-or-nothing: `403` unless the caller holds `Tier.VIEWER` on every study each selected prep_sample is linked to (wet_lab_admin+ bypass; a named `study_idx` or `prep_sample_idx` is checked before any lookup; an unknown study selects nothing); `409` when the mask ran without SynDNA, a named prep_sample is not masked under it, a selected prep_sample is not `completed`, or a completed one has no counts (naming `qiita-admin backfill syndna-read-count`); `404` for an unknown mask or an empty selection; `413` above the roster cap. Requires `prep_sample:read`. |

### Run folder

| Route | Method | Notes |
|---|---|---|
| `/api/v1/run-folder/inspect` | POST | Read a sequencing run folder on the shared filesystem and return what a submit gesture needs from it: an Illumina run's `instrument_run_id` / `instrument_model` (from `RunInfo.xml`), or a PacBio run's barcode → HiFi BAM index. `require_role_at_least(wet_lab_admin)` and **no scope** — the gate is deliberately the same role that may name a host path in a work ticket's `action_context` (`_check_ingest_paths`), because the answer is only useful to someone who can then submit against that path. Read-only, and it returns facts rather than bytes: no file content crosses the wire. The path is bounded by `PATH_INGEST_ROOTS` via the same `resolve_ingest_path` the submit gate uses, so this cannot be used to probe the filesystem outside those roots. POST rather than GET because the body carries an absolute path, which in a query string would land in access logs and proxy caches. **403 vs 404 matters here:** the control plane runs as `qiita-api` and SLURM steps as `qiita-job`, with different group membership, so a folder the job can read may be one the control plane cannot — that case answers 403 naming the situation, never 404, which would send an operator hunting for a typo in a path that is correct. |

### Exported identifier

| Route | Method | Notes |
|---|---|---|
| `/api/v1/exported-identifier` | POST | Mint (or recover) the public `export_id` (`QM<n>`) for each processed sample — `(alignment_idx, prep_sample_idx)` — in a cohort, so a published table can name its samples without carrying ours. `require_human` + `prep_sample:read`, and **that read scope covers a write on purpose**: the row it creates is a name, the mint is idempotent, it exposes nothing the caller could not already see, and the authority it takes is exactly "may I read this sample" — a separate write scope would have to be granted to precisely the roles that can already read one. No complete-profile requirement, unlike the alignment mint, which hands out access to raw sequence-derived data. The **same** all-or-nothing gate as that mint (`filter_prep_samples_caller_can_read` at `Tier.VIEWER` on every study a sample is still linked to, `wet_lab_admin` bypassing), so the two cannot disagree about who may read a sample. Refusals are ordered alignment `404` → access `403` → completeness `422`, mirroring the mint exactly; the `403` is truncated and uncorrelated for the reason given there, and the `422` subsumes an unknown `prep_sample_idx` (no gate row) so a typo cannot vanish from an all-or-nothing answer. A refusal mints nothing. POST because a cohort spans pools and would exceed nginx's request-line cap in query params. No new scope. |
...
| `reference exclusion add (--genome-idx N \| --feature-idx N) --reason R` / `remove (…)` / `sync` | HTTP | The write surface for the global reference blocklist, via the `/reference/exclusion` routes above. `add`/`remove` take a required, mutually-exclusive target; `sync` takes none (force-resync the mirror). Requires a PAT carrying `reference:exclusion:write`. The read-only `list` lives under the `qiita` CLI (below), since a `reference:read` query any user can run belongs there. |
| `terminology robot-command --input FILE [--export FILE] [--executable CMD]` | local | Prints the ROBOT `export` command to run against a staged OWL file, requesting the columns the OWL extractor reads (`ID`, `LABEL`, `owl:deprecated`, `IAO:0100001`, `oboInOwl:hasAlternativeId`) over `--include classes`. Prints only — nothing in the control plane executes ROBOT, on any host, so the operator runs the printed command themselves before `prepare-owl`. `--executable` (default `robot`) takes a whole command, so a container runtime's tokens can lead it. Intended to be temporary, until ROBOT runs inside the apptainer/workflow/slurm pipeline like every other external tool. |
| `terminology prepare-owl --export FILE --name N --version V [--term-id-prefix P] [--output-dir DIR]` / `prepare-taxdump --taxdump FILE --name N --version V [--output-dir DIR]` | local files | Turns a release source into the three files `load` consumes: `terms.tsv`, `closure.tsv`, and `manifest.json` declaring both digests. `--output-dir` defaults beside the input and is created if absent (exit 2 if it cannot be); a source that cannot be read is exit 1, leaving no directory behind. `prepare-owl` reads a ROBOT export, and `--term-id-prefix` keeps only that prefix's term ids so classes the source imports from other vocabularies stay out — a replacement pointer reaching outside the prefix is dropped with a warning, since a replacement resolves within one terminology. `prepare-taxdump` reads a `taxdump.tar.gz` (or `new_taxdump.tar.gz`) in place with nothing unpacked, through duckdb-miint's taxdump readers — the archive must be a gzipped tar carrying its `.dmp` members at the top level, and a directory of extracted members is refused. A live taxon takes its scientific name as `label` and its genbank common name as `alternate_label`, a merged taxon becomes obsolete pointing at its survivor, and a deleted one becomes obsolete with no replacement; term rows land in taxon-id order, so the same archive always yields the same digest. Until closure calculation is implemented, **the closure table is always written header-only**, so a loaded terminology resolves terms but answers no subsumption queries. |
| `terminology load --manifest FILE --terms FILE --closure FILE [--tolerate-anomalies]` | direct DB | Applies a prepared release in one transaction, ending with the `qiita.terminology` row `active`, and prints how many terms were inserted, relabelled, obsoleted, or merged. The three files are named individually and copied into a temporary directory, so no staging directory has to exist on the host; both tables are verified against the manifest before either is parsed, so a file that does not match is refused before any row is written. Reloading over an existing release leaves a term row alone when none of its stored values differ, except that a term carrying a replacement pointer is rewritten every load and the closure table is rebuilt whole. `--tolerate-anomalies` absorbs a release that silently dropped terms (auto-obsoleting them), records an unresolvable replacement pointer as a note, and drops a closure row naming an endpoint the release does not define; a live term carrying a replacement pointer always refuses the load either way. Reads `DATABASE_URL` from env (exit 2 when unset). |
...
| `assembly export --processing-idx N (--prep-sample-idx P \| --sequenced-pool-idx P \| --study-idx S) [--kind LCG\|MAG\|UNBINNED …] [--min-bp/--max-bp B] [--min-completeness/--max-contamination PCT] --output-dir D --data-plane-url U` | HTTP + Flight | One gzipped FASTA per genome, `<biosample accession>_<bin_id>.fasta.gz`, plus `genomes.tsv` and `contigs.tsv`; no internal identifier in any of them. Walks `GET /assembly/{N}/prep-sample`, then per sample the membership Parquet and three run mints (`assembled_sequence`, `bin_quality`, `assembled_sequence_chunks`). Refuses, writing nothing, on a `pending`/`invalidated` prep_sample, a prep_sample whose biosample has no accession, a name repeated in one export, membership and stream disagreeing, or a record whose reassembled length differs from `sequence_length_bp`. Default kinds LCG and MAG. |
| `mask syndna-read-count --mask-idx M (--study-idx S \| --sequenced-pool-idx P \| --prep-sample-idx N …) --output F [--format biom\|parquet] [--prefix-pool] --data-plane-url U [--feature-names species\|accession]` | HTTP + Flight (Flight for `species`, the default) | Calls `GET /mask-definition/{M}/syndna-read-count` and writes `(sample_id, feature_id, value)` as BIOM (default; zero cells omitted) or Parquet. Each prep_sample's `sample_id` is its biosample accession, or `<sequenced_pool_idx>_<accession>` under `--prefix-pool`; inserts by the reference taxonomy's `species` over a reference DoGet (default), or by the recorded FASTA header with `--feature-names accession`. Refuses, writing nothing, on a prep_sample with no accession, a name two prep_samples would share, an insert with no name, or an existing `--output`. |
