...
| `mask syndna-read-count --mask-idx M (--study-idx S \| --sequenced-pool-idx P \| --prep-sample-idx N …) --output F [--format biom\|parquet] [--prefix-pool] [--feature-names accession\|species --data-plane-url U]` | HTTP (+ Flight for `species`) | Calls `GET /mask-definition/{M}/syndna-read-count` and writes `(sample_id, feature_id, value)` as BIOM (default; zero cells omitted) or Parquet. Each prep_sample's `sample_id` is its biosample accession, or `<sequenced_pool_idx>_<accession>` under `--prefix-pool`; inserts by the recorded FASTA header, or by the reference taxonomy's `species` over a reference DoGet. Refuses, writing nothing, on a prep_sample with no accession, a name two prep_samples would share, an insert with no name, or an existing `--output`. |
