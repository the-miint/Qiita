...
| `/api/v1/assembly/{prep_sample_idx}/{processing_idx}/membership` | GET | Every `qiita.assembly_membership` row of one run — `feature_idx`, `kind`, `bin_id` and the assembler's `raw_name` / `circularity` / `depth` / `mult` — every kind including UNBINNED, ordered by `(kind, bin_id, feature_idx)`. The genome map's gates without its `422`: no `genome_idx` is returned, so an unminted row is not a gap here. `413` above its own cap, sized to the genome map's body budget. The Postgres copy, which keeps a superseded row where the lake replaced it, so `qiita assembly export` compares these rows with what the data plane streams and refuses on a difference. Requires `prep_sample:read`. |
