...
The de novo arm: two DoGet mints over one assembly run's contigs, and the run-scoped
`feature_idx` → `genome_idx` map beside them. Why `assembly:doget` sits where it does,
and why the map is not a ticket at all, is in the Scopes and roles section above.
