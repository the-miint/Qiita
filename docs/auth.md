...
| `/api/v1/mask-definition/{mask_idx}/syndna-read-count` | GET | Per-insert SynDNA read counts under one mask for the samples selected by `study_idx`, `sequenced_pool_idx` and/or repeatable `prep_sample_idx` (intersected; at least one required, else `422`): the reference's inserts with their recorded accession, and per sample its biosample accession, `sequenced_pool_idx` and one count per insert. All-or-nothing: `403` unless the caller holds `Tier.VIEWER` on every study each selected sample is linked to (wet_lab_admin+ bypass; a named `prep_sample_idx` is checked before any lookup); `409` when the mask ran without SynDNA, a named sample is not masked under it, a selected sample is not `completed`, or a completed one has no counts (naming `qiita-admin backfill syndna-read-count`); `404` for an unknown mask or an empty selection; `413` above the roster cap. Requires `prep_sample:read`. |
