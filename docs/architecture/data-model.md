...
`read` follows its own rule: `register_files` **replaces** the `read` rows a work ticket registered before for the prep_samples it registers again, in the same transaction, and leaves rows other tickets registered as they are. `register_files` carries why.
