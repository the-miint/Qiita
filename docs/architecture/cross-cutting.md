...
**Horizontal scaling**: each data plane instance holds an independent DuckDB+DuckLake connection to the shared Postgres catalog. Each request opens its own connection and `ATTACH`, so a read through one instance sees a commit made through another: measured 100 of 100 for both `INSERT` and `ducklake_add_data_files`, on DuckDB 1.5.4 with ducklake `d318a545`, two processes each opening a fresh in-memory connection per operation against one Postgres catalog. Writes do contend: registrations into the replace-keyed tables serialize on `qiita_lake.registration_lock` (`flight_service::register_files`), and instances booting together conflict on the catalog, which `ducklake::setup_catalog` retries. How instances and peers are configured and rendered into nginx is in [build-and-deploy.md](build-and-deploy.md#deployment).
