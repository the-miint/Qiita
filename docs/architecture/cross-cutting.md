...
- **Data plane Unix user and file protection:** The data plane runs as the dedicated `qiita-data` system user (no login shell). SLURM jobs run as `qiita-job` and write each step's outputs into the per-ticket workspace, `PATH_SCRATCH/ticket/<work_ticket_idx>/<step>/attempt-<N>/` (under SLURM its `output/` subdirectory, `$QIITA_OUTPUT_PATH`), setting every output file to mode `440` — owner and group read-only. The orchestrator checks that mode after the job exits (`slurm/verify.py`, gate 3) and fails the step as a contract violation if any file differs. Jobs never write into the lake: registration (`register_files` in `qiita-data-plane/src/flight_service.rs`) is the data plane moving each registered file from the step's staging dir into `PATH_PERSISTENT/ducklake/<table>/` under a name it mints, refusing to overwrite an existing file, and then calling `ducklake_add_data_files`. The data plane reads files it does not own through the shared `qiita-pipeline` group; the durable data dir is group `qiita-data` at mode `0750`, so `qiita-job` cannot read it (the groups table in [`first-deploy.md`](../runbooks/first-deploy.md)).
