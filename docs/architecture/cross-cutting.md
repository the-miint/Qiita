...
- **Shared filesystem paths:** Three base roots — `PATH_PERSISTENT` (durable), `PATH_SCRATCH` (working) and `PATH_DERIVED` (built artifacts). Canonical layout and retention policy in [Data Storage](storage.md#data-storage). Each component accesses only the roots it derives paths from; the same section says which component derives which.
...
- **Data plane Unix user and file protection:** The data plane runs as the dedicated `qiita-data` system user (no login shell). SLURM jobs run as `qiita-job` and write each step's outputs into the per-ticket workspace, `PATH_SCRATCH/ticket/<work_ticket_idx>/<step>/attempt-<N>/` (under SLURM its `output/` subdirectory, `$QIITA_OUTPUT_PATH`), setting every output file to mode `440` — owner and group read-only. The orchestrator checks that mode after the job exits (`slurm/verify.py`, gate 3) and fails the step as a contract violation if any file differs (`EXPECTED_FILE_MODE`, `slurm/contract.py`). Jobs never write into the lake: registration (`register_files` in `qiita-data-plane/src/flight_service.rs`) is the data plane moving each registered file from the producing step's output dir into `PATH_PERSISTENT/ducklake/<table>/` under a name it mints, refusing to overwrite an existing file, and then calling `ducklake_add_data_files`. The data plane reads files it does not own through the shared `qiita-pipeline` group; the durable data dir is group `qiita-data` at mode `0750`, so `qiita-job` cannot read it (the groups table in [`first-deploy.md`](../runbooks/first-deploy.md)).
- **Data plane horizontal scaling:** The data plane is the read/write path for all DuckLake data. At scale, many concurrent SLURM jobs may issue large DoGet reads simultaneously, making a single data plane process a throughput bottleneck. The data plane scales horizontally because each instance is stateless with respect to request handling — it holds only a DuckDB+DuckLake connection to the shared Postgres catalog and reads Parquet files from the shared filesystem. DuckLake's concurrent read model is safe for this: multiple DuckDB instances connecting to the same Postgres catalog never block each other (readers use snapshot isolation with no row-level locking; conflicts only arise on concurrent writes, resolved via optimistic concurrency at commit time). Workers cannot bypass the data plane and read Parquet files directly for several reasons: (1) deletions are recorded as separate delete files in the DuckLake catalog — raw Parquet reads return logically deleted rows; (2) small inserts below the data inlining threshold are stored entirely within the Postgres catalog (`ducklake_inlined_data_tables`), with no Parquet file written at all; (3) snapshot visibility requires a catalog query to determine which files are live for the current consistent state — files that are not live stay on disk until [`scripts/lake-gc.sh`](../../scripts/lake-gc.sh) reclaims them (its header lists how they arise), so a glob over a table's directory reads files the catalog does not list, and deduplicating the result does not recover the catalog's answer when those files hold a different run; (4) compaction rewrites and deletes Parquet files under active management, making cached paths unreliable. For jobs and services, the data plane remains the correct and only correct read path (ad-hoc inspection by hand goes through the read-only catalog attach in [`scripts/lake-shell.sh`](../../scripts/lake-shell.sh)); the solution to the bottleneck is running more data plane instances.
