...
- **Shared filesystem paths:** Three base roots — `PATH_PERSISTENT` (durable), `PATH_SCRATCH` (working) and `PATH_DERIVED` (built artifacts). Canonical layout and retention policy in [Data Storage](storage.md#data-storage). All components — control plane, data plane, SLURM jobs — access both mounts.
