# Deployed 2026-09-03 — a222cc99

Two sources: #484 (ingest-path roots, the run-folder inspect route, and upload-fed
read submission) supplies buckets 1, 2, 3 and 5; #525 supplies the last Notes entry
(`checkm` baseline memory). Buckets 4 and 6 were empty for this deploy.
...
([`reference-data-staging.md`](../reference-data-staging.md)) — a scratch mount, not a
...
- **The ingest-root gate is live and bounded.** A path outside the roots must be refused at
  ...
  with a PAT (see [`user-cli-quickstart.md`](../runbooks/user-cli-quickstart.md)):
  ```bash
