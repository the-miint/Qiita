...
> **Correction (2026-08-25).** The claim below that the 24 uncollapsed
> `reference_sequence_chunks` features are reverse-complement pairs needing their
> producing reference load re-run is wrong for 23 of them, which differ only in
> soft-masking case. Case is normalized at the split from (#479) on, so those 23
> are byte-identical under normalization and collapse unambiguously; `feature_idx`
> 127 is the one true reverse complement and stays ambiguous. The measurements and
> the full account are in issue #473. The text below is left as written.
