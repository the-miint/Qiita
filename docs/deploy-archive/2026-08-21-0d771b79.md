...
> **Correction (2026-08-27).** The `shear_tree` probe below runs `python3`, which as
> `qiita-api` is the system interpreter and has no `duckdb` module — it exits with
> `ModuleNotFoundError` rather than printing a count. Use the control plane's own
> interpreter, the one it LOADs miint from: `/opt/qiita/control-plane/.venv/bin/python`
> (`deploy/activate.sh`), with `deploy/preflight.sh`'s fallback to
> `<checkout>/qiita-control-plane/.venv/bin/python`. Observed 2026-08-27 on the
> identically-shaped `circular_query_coverage` probe in
> [`2026-08-27-7cf8816a.md`](2026-08-27-7cf8816a.md) §5.
...
> **Correction (2026-08-27).** The pgpass step below does not work when the catalog
> connection string carries a password: it writes `~/pgpass` as the operator at mode
> 0600, then runs `duckdb` as `qiita-data`, which cannot read a 0600 file in another
> account's home. On the deploy host `DUCKLAKE_CATALOG_CONNSTR` does carry one
> (measured 2026-08-27), so `PGPASSFILE` is consulted rather than ignored. What libpq
> does with the unreadable file was not observed — the 2026-08-27 run created the
> pgpass and the SQL file *as* `qiita-data` instead (`printf … | sudo -u qiita-data
> tee`, into a 0700 directory under `/tmp` removed on exit), which keeps the password
> out of argv and out of any file the operator owns. The rest of the scaffolding —
> the run-as, `SET home_directory`, `-bail` as the CLI flag, and the quiesce
> requirement — is unchanged. See [`2026-08-27-7cf8816a.md`](2026-08-27-7cf8816a.md) §6.
