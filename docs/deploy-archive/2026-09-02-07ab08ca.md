# Deployed 2026-09-02 — 07ab08ca

Stamped while buckets 5 and 6 were still in flight, at the maintainer's request: 5a–5c
passed, the bucket-5 pilot assembly (work ticket 8281, prep_sample 30457) was still in its
`assemble` step, and bucket 6's 26 re-runs had not started. The commands below are the
corrected ones — four of them did not run as written and were fixed in `7a614530` before
this archive was cut, so this file records what works rather than what was first written.
