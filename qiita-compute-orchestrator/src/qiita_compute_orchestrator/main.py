...
def _build_backend(settings: Settings) -> ComputeBackend:
    ...
    if settings.backend_type == BACKEND_SLURM:
        ...
        return SlurmBackend(
            ...
            # Forward already-resolved tokens + CP URL so SLURM jobs
            # can re-resolve Settings.from_env() on the compute node
            # without reading deploy-host-local /etc/qiita/*.token.
