...
def test_build_bowtie2_index_real_smoke(tmp_path, monkeypatch):
    ...
    This is the AUTHORITATIVE miint verification for the bowtie2 builder:
      ...
      - runs against real miint with the GPL boundary pre-installed by the session
        staging fixture (mirroring the deploy, which installs it once — the seam no
        longer installs it per job).
