...
class LibraryPrimitive(StrEnum):
    ...
    # delete-alignment-block. Runs immediately BEFORE register-files on a
    # prep_sample-scoped ticket, deleting the (alignment_idx, prep_sample_idx)
    # pair's rows so a re-run deletes-then-re-registers without double-counting.
    # See qiita_control_plane.actions.library.delete_alignment_sample.
