...
class ReadMaskedDoGetTicketRequest(BaseModel):
    ...
    they are the macro's required arguments, so an unscoped masked read is
    unrepresentable in the data plane; the route still refuses to sign one, as
    defence in depth (the mandatory-filter invariant).
    """
