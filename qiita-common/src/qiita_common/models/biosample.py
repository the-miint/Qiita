...
class BiosampleStudyFieldCreateRequest(BaseModel):
    """Body for POST /api/v1/study/{study_idx}/biosample-field — mints a
    study-local biosample field definition (no metadata value is written).
    ...
    biosample_global_field_idx: Annotated[int, Field(gt=0)] | None = None
    ...
    def _validate_mode_coupling(self) -> BiosampleStudyFieldCreateRequest:
        ...
        if self.biosample_global_field_idx is not None:
            forbidden = [
                name
            ...
            if forbidden:
                raise ValueError(
                    "biosample_global_field_idx links to a global field; "
        ...
        if self.data_type is None:
            raise ValueError("data_type is required when biosample_global_field_idx is omitted")
...
class BiosampleStudyFieldResponse(BaseModel):
    """Returned by POST /api/v1/study/{study_idx}/biosample-field (201 body is
    the created resource).

    Carries every qiita.biosample_study_field column. For a globally-linked
    row, data_type / required / terminology_idx are the values inherited from
    the global-field row (resolved at read time), so they are always populated
    even though the study-field columns are NULL. Global-field-only columns
    (internal_name, default_tier) belong to the global-field resource and are
    excluded.
    """

    biosample_study_field_idx: Annotated[int, Field(gt=0)]
    study_idx: Annotated[int, Field(gt=0)]
    biosample_global_field_idx: Annotated[int, Field(gt=0)] | None
    display_name: str
    description: str | None
    data_type: FieldDataType
    required: bool
