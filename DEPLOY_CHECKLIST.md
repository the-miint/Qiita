...
- `20260721000000_metadata_global_field_alias_index_comments.sql` — attaches explanatory COMMENTs to the per-global metadata uniqueness indexes (no schema change). Plain `make migrate`. (#feat_biosample_metadata_import)
- `20260721000001_global_field_display_name_unique.sql` — adds `UNIQUE(display_name)` to both global-field tables. Plain `make migrate` — **but only after the bucket-2 pre-check passes** (the constraint build aborts on existing duplicate display_names). (#feat_biosample_metadata_import)
