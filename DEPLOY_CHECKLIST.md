...
- `20260714120000_coverage_definition.sql` — adds `qiita.coverage_definition` (the CP-minted params-hash identity of a coverage measurement, twin of `alignment_definition`) + the `mint_coverage_definition` function. Plain additive, standard `make migrate`, no backfill: nothing writes coverage until a `syndna_enabled` read-mask runs against an annotated reference. (#269 part 2)
...
- **The DuckLake `coverage` table is created automatically at data-plane startup** by `ensure_coverage_tables` (idempotent, every DP boot) — **no data-plane action, no migration**. It is the per-`(prep_sample, feature)` mean-depth feature table; a sink for now (not in `ALLOWED_TABLES`, like `alignment`), so no Flight read-side yet. (#269 part 2)
- **The `read-mask` workflow gains a `coverage_depth` step + a `delete-coverage`/`register-files` pair, all under the existing `syndna_enabled` gate** — reaches `qiita.action` via `qiita-admin actions sync` (in `activate.sh`), so **no host action**. Inert unless a pool is masked with `syndna_enabled`; a read-mask run without it is byte-identical to today. To produce real per-insert numbers the SynDNA reference must be re-ingested as **plasmids + a per-insert GFF3** (the current reference is bare inserts, and the 0.90 aligned-fraction gate is only correct against plasmids) — that is a separate operator action, filed when the reference is rebuilt, not part of this deploy. (#269 part 2)
