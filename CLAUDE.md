...
- `genome_idx` = logical entity across references (nullable — not all features are genomes, e.g., 16S records). Keyed by `(source, source_id)`; whether that `source_id` is an external accession or an internal name is per-`source`, and `repositories/exported_feature.py` is the authority. `source='qiita'` is one of our own assemblies
