...
**Horizontal scaling**: each data plane instance holds an independent DuckDB+DuckLake connection to the shared Postgres catalog. DuckLake's snapshot isolation means instances never block each other. Scale with the two deploy-time env vars, never by hand-editing nginx — `activate.sh` overwrites `/etc/nginx/conf.d/qiita.conf` from the checked-in file on every deploy, so a hand-added upstream member survives exactly until the next one. `QIITA_DATA_PLANE_PORTS` adds instances on *this* host (it also drives the matching `qiita-data-plane@NNNN` systemd units); `QIITA_DATA_PLANE_PEERS` adds data planes running on *other* hosts as upstream members only. A peer must share the same catalog and scratch/persistent paths, and is reached over plaintext gRPC — see the deploy checklist.
