...
**Merge, don't append.** Folding means adding your line into the *existing* bucket block (e.g. one more `KEY=value` inside the compute-orchestrator `tee`), never starting a parallel block. Reviewers check that an operator-impacting PR folded its steps in and that buckets weren't duplicated. The deploy/archive procedure itself — how the operator runs `## Pending deploy` and archives it via `/deploy-archive` — lives in [`docs/runbooks/redeploy.md`](docs/runbooks/redeploy.md), the single source of truth for that lifecycle; don't restate it here or in the CHANGELOG.
