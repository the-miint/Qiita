...
data plane. Ad-hoc inspection by hand uses a `READ_ONLY` attach of the catalog,
`make lake-shell`, which is admin-only and not a data-access path (see the header of
[`scripts/lake-shell.sh`](scripts/lake-shell.sh)). If the catalog is not reachable where
the work runs, stop rather than fall back to the files. The reasons are in
