...
def cp_server(tmp_path, hmac_secret):
    ...
    # Settings.from_env() requires WORK_TICKET_WORKSPACE_ROOT and
    # UPLOAD_STAGING_ROOT — the CP would fail to boot without them. The
    # dirs don't need to exist for this smoke (the dispatch points at a
    # dead orchestrator port, so the runner never reaches mkdir); the
    # values just need to be absolute paths so the boot-time validation
    # passes.
    env = {
        **os.environ,
