"""Full-span integration test: the batch multi-study ENA import driver
threaded end-to-end into a real DuckLake `read` table, in one process.

Chains together, for real, three pieces every other suite exercises only in
isolation:

  - the batch driver (`ena_import.batch.create_ena_import_batch` /
    `_process_one_study`) — real Postgres registration
    (`ena_import.registration.register_ena_study`) and a real submitted
    `download-ena-study` work_ticket row, exactly as `POST
    /ena-import-batch` drives it (see `qiita-control-plane/tests/
    ena_import/test_batch.py`, which covers this layer in isolation);
  - the download-and-store native job (`ingest_ena_reads.execute`) fed the
    SAME roster the runner itself would stage for that ticket
    (`repositories.sequenced_sample.fetch_sequenced_pool_run_roster`), with
    the whole ENA network fetch (`_stage_run_reads`) and the sequence_idx
    mint (`sequence_range_retry.mint_sequence_range`) monkeypatched exactly
    as `test_ena_ingest_e2e.py` does it — no network, real CP mint, real
    read-storage pipeline;
  - the real `register-files` action against a real data-plane process and
    real DuckLake catalog (also mirroring `test_ena_ingest_e2e.py`).

The resolver seam (`miint_resolver._query_ena_*`) is network-free and
fixture-driven, mirroring `test_batch.py`'s own `_monkeypatch_resolver_seam`
— duplicated here rather than imported, matching this codebase's existing
convention of small per-suite test-data duplication (see
`qiita-compute-orchestrator/tests/test_ingest_ena_reads.py` vs.
`test_ena_ingest_e2e.py`'s own `_write_run_map`/`_write_intermediate`
duplication) rather than reaching into another package's test tree.

The one fixture study here resolves two runs that share ONE sample
accession, on the same platform — the study-level shape that most exercises
the FULL span at once: one `sequenced_pool` (both runs land in it, so one
`ingest_ena_reads` ticket covers both) and one de-duplicated biosample (so
the DuckLake read counts, once written, can be checked against exactly two
`prep_sample_idx` values sharing a single biosample identity).
"""
...
# Network-free resolver seam -- same target strings as
# qiita-control-plane/tests/ena_import/test_batch.py.
...
def _make_two_runs_sharing_one_sample(shared_sample_accession: str):
    """Build a `(runs, attrs)` fake-resolver pair for one study whose two
    runs share ONE `sample_accession` -- both ILLUMINA (a single
    `sequenced_pool`), so the full span exercises intra-study biosample
    de-dup as it lands in DuckLake."""
...
def _download_ena_study_action_entries():
    ...
    WorkflowAction entries in declared order -- mirrors
    test_ena_ingest_e2e.py's `_download_ena_study_action_entries`."""
...
def _write_run_map(path: Path, roster: list[tuple[int, str]]) -> None:
    ...
    test_ena_ingest_e2e.py's `_write_run_map`."""
...
def _write_intermediate(
    ...
    """Write the `_stage_run_reads` intermediate shape -- verbatim from
    test_ena_ingest_e2e.py's `_write_intermediate`."""
    with duckdb.connect(":memory:") as conn:
        values = ", ".join(
...
def _fake_stage_run_reads_factory(by_run: dict[str, tuple[list[tuple], list[str]]]):
    ...
    verbatim shape from test_ena_ingest_e2e.py's own factory."""
...
async def _run_register_files(
    ...
    """Drive the REAL runner adapter for the download-ena-study
    `register-files` entry -- verbatim from test_ena_ingest_e2e.py's
    `_run_register_files`."""
...
def _patch_run_and_log(monkeypatch):
    ...
    -- this test drives its native step directly (`ingest_ena_reads.
    execute()`), not through a live orchestrator. Mirrors test_batch.py's
...
async def batch_app(postgres_pool):
    ...
    batch driver -- mirrors test_batch.py's `batch_app` fixture."""
...
async def admin_principal(postgres_pool):
    """A real seeded wet_lab_admin principal -- mirrors test_batch.py's
    `admin_principal` fixture."""
...
async def download_ena_study_action(postgres_pool):
    ...
    `submit_work_ticket_core` can resolve it -- mirrors test_batch.py's
...
async def _cleanup_study(postgres_pool, study_accession: str) -> None:
    """FK-reverse cleanup for the one study this test creates -- mirrors
    test_batch.py's `_cleanup_study`."""
...
async def test_batch_driver_to_register_files_to_ducklake_full_span(
    ...
    # -------------------------------------------------------------------
    # Idempotency: re-run _process_one_study for the SAME item. Registration
    # is idempotent (both runs are already `sequenced_sample` rows, so
    # register_ena_study skips them; the study/biosample already exist) --
    # no new study or biosample row is created, and since this second call
    # never touches ingest_ena_reads/register-files again, the DuckLake read
    # count must be exactly unchanged too.
    # -------------------------------------------------------------------
    await _process_one_study(
        batch_app,
        postgres_pool,
