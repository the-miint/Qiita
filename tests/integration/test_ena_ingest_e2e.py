"""Integration test: prove ENA-fetched reads land in DuckLake `read` through the
EXISTING tail — `ingest_ena_reads.execute()` -> real `register-files` primitive
-> real data-plane `register_files` DoAction -> real DuckLake catalog. No product
code changes accompany this file; the tail already works and this is the proof.

execute() is called directly, then the real register-files tail is driven. Only
two seams are faked: `_stage_run_reads` (the whole `read_ena_sequences` seam —
ENA resolution + HTTP download) is replaced with an inline DuckDB COPY (no
network, no fixture), and `mint_sequence_range` calls `qiita.mint_sequence_range`
directly via `postgres_pool` so the minted range is REAL. Everything else
(write_sorted_reads/hardlink, register-files against a real data plane) is real.

Parity note: `ingest_ena_reads` and `ingest_reads` share
`read_staging.write_sorted_reads`/`hardlink` verbatim, so the "identical to the
bcl-convert path" claim is CODE IDENTITY. The real ENA network fetch is out of
scope here — that is `test_ingest_ena_reads.py`'s job (plus its live smoke).
"""
...
def _download_ena_study_action_entries():
    ...
    WorkflowAction entries in declared order, so a future reorder of the tail is
    reflected automatically rather than drifting from a hand-copied entry."""
...
def _write_intermediate(
    ...
    """Write the `_stage_run_reads` intermediate shape `(sequence_index, read_id,
    sequence1, qual1, sequence2, qual2)` — the exact 6-column projection
    `_stage_run_reads` itself copies."""
    with duckdb.connect(":memory:") as conn:
        values = ", ".join(
...
def _fake_stage_run_reads_factory(by_run: dict[str, tuple[list[tuple], list[str]]]):
    """Build a `_stage_run_reads`-shaped fake keyed by run_accession; each entry
    is `(rows, warnings)`."""
...
async def _run_register_files(
    ...
    entry — the exact production tail `ingest_ena_reads` hands its output to."""
...
async def test_ena_reads_land_in_ducklake_read_via_register_files(
    ...
    `hardlink` pipeline, and registered into the real DuckLake `read` table.
    Then proves STORED ONCE: re-registering under the SAME work_ticket_idx is
    refused by the data plane (colliding lake filename), row count unchanged."""
    ...
    # function directly through the existing postgres_pool.
    ...
    # Prove STORED ONCE. Re-execute against the SAME reads_staging_root: the
    # durable read.parquet already exists, so execute() takes its idempotent fast
    # path and re-creates the register part in a FRESH workspace. Re-running
    # register-files with the SAME work_ticket_idx is refused, because the data
    # plane mints the lake filename deterministically from (work_ticket_idx,
    # basename) and refuses to overwrite an existing one — a REFUSAL, not a silent
    # no-op (the read table has no uniqueness constraint, so nothing else would
    # stop a different-ticket resubmit; that is the CP's disallow-without-delete
    # gate's job, one layer up).
    #
    # Exception type: register_files's docstring says pyarrow.flight.FlightError,
    # but a DoAction error with no dedicated Flight status mapping surfaces as a
    # plain pyarrow.lib.ArrowException ("Unknown: ...") — imprecise docstring,
