...
is the DP-side DoGet + the orchestrator's stream_reference_chunks consumer.
...
def test_stream_reference_chunks_scoped_to_feature_subset(data_plane):
    ...
    try:
        with stream_reference_chunks(
            conn, data_plane_url=url, ticket_bytes=ticket
        ) as rel:
            seqs = dict(
                conn.execute(
