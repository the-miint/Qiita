...
def test_parse_robot_export_missing_column(tmp_path):
    ...
    with pytest.raises(duckdb.Error, match="Expected Number of Columns: 5 Found: 2"):
        parse_robot_export(export_path)


def test_parse_robot_export_reordered_header(tmp_path):
    """Tests the case where the export names every requested column but in
    another order: the parse refuses it, since the read maps columns
    positionally and each cell would otherwise land under the wrong one."""
    export_path = tmp_path / "robot-export.tsv"
    reordered = (ROBOT_EXPORT_HEADER[1], ROBOT_EXPORT_HEADER[0], *ROBOT_EXPORT_HEADER[2:])
    header = "\t".join(reordered)
    export_path.write_text(f"{header}\nmouth\tUBERON:0001\t\t\t\n")
...
def test_parse_robot_export_no_term_id(tmp_path):
    """Tests the case where a row carries no term id: the parse refuses it,
    naming the column and the row it is on."""
    export_path = tmp_path / "robot-export.tsv"
    write_robot_export_tsv(export_path, [("", "mouth", "", "", "")])

    with pytest.raises(ValueError, match="row 1 carries an empty ID"):
        parse_robot_export(export_path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("UBERON:0001\tmouth\t\t\t\tEXTRA\n", 6),
    ],
    ids=["stops_early", "stops_before_the_last_cell", "runs_past_the_last_column"],
)
def test_parse_robot_export_row_of_wrong_width(tmp_path, row_text, cells_found):
    """Tests the case where a row does not carry one cell per requested column:
    the parse refuses it, naming the line and both counts, since the cells are
    positional and either way the values would sit under the wrong columns."""
    export_path = tmp_path / "robot-export.tsv"
    header = "\t".join(ROBOT_EXPORT_HEADER)
    export_path.write_text(f"{header}\n{row_text}")

    expected_error = f"(?s)Line: 2.*Expected Number of Columns: 5 Found: {cells_found}"
    with pytest.raises(duckdb.Error, match=expected_error):
        parse_robot_export(export_path)


def test_parse_robot_export_blank_and_padded_label(tmp_path):
    """Tests the case where a label carries only whitespace or is padded: the
    first arrives empty and the second stripped, alongside a label needing
    neither."""
    export_path = tmp_path / "robot-export.tsv"
    write_robot_export_tsv(
        export_path,
    ...
    expected = [
        ...
        exported_class("UBERON:0002", "molar"),
        exported_class("UBERON:0003", "mouth"),
    ]
    assert result == expected
