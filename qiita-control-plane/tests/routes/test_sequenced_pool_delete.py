...
real Postgres. There is no data-plane call to stub (the pool delete is
Postgres-only today — DuckLake result tables keyed by prep_sample/processing
don't exist yet), so these tests focus on what the control plane owns:
authorization, FK-ordered cascade, biosample/run retention, and the
work-ticket / publication / ENA gating.
"""
