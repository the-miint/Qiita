...
async def test_post_study_full_body_round_trips(ctx):
    ...
    assert rj["title"] == title
    assert rj["alias"] == "p-1"
    assert rj["description"] == "desc"
    assert rj["abstract"] == "abs"
    assert rj["funding"] == "NIH-R01"
    assert rj["ebi_study_accession"] == "ERP000001"
    assert rj["vamps_id"] == "VAMPS-1"
    assert rj["notes"] == "notes-1"
    assert rj["extra_metadata"] == extra
    assert rj["default_tier"] == "viewer"
