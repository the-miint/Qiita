"""Tests for the ENA-import provenance surface on qiita.sequenced_sample: the
source_archive/resolver_kind/transport kwargs on insert_sequenced_sample,
fetch_sequenced_sample_idxs_by_ena_run_accession (idempotent-re-import lookup),
and set_sequenced_pool_transport. Each test seeds and asserts inside one
...
async def _seed_pool_with_samples(conn, owner, count: int) -> tuple[int, list[int]]:
    """Seed ONE sequenced_pool with `count` sequenced_sample rows (each its own
    biosample/prep_sample), all sharing the pool, so
    set_sequenced_pool_transport's pool-scoped UPDATE has more than one row to
    act on. Returns (sequenced_pool_idx, [sequenced_sample_idx, ...])."""
    protocol_idx = await conn.fetchval(
        "SELECT idx FROM qiita.prep_protocol WHERE name = 'short_read_metagenomics'"
    )
    run_idx, _ = await insert_sequencing_run(
