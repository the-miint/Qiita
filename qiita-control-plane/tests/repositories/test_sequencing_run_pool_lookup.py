...
(ena_import.registration's get-or-create-pool-for-run step). Each test runs
inside one open transaction as the seed writes.
"""
...
async def test_fetch_sequenced_pool_idxs_for_run_returns_oldest_first(postgres_pool):
    async with postgres_pool.acquire() as conn:
        ...
        try:
            ...
            # Inserts always create a fresh row (no natural content key to
            # ON-CONFLICT against) -- why the registration composer fetches first.
