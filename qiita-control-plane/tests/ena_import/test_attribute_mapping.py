"""Tests for `ena_import.attribute_mapping.map_ena_attributes`: curated ENA tag ->
...
def test_map_ena_attributes_leaves_host_unmapped():
    """`host` is free-text, not an NCBI taxon id -- taxon resolution is out of scope,
    so it stays unmapped rather than fabricating a `host_taxon_id`."""
...
def test_map_ena_attributes_leaves_environmental_context_triad_unmapped():
    """The environmental-context triad is ENVO-typed; mapping ENA's free text would
    need an ENVO-CURIE resolution out of scope here, so they stay unmapped (like taxon)."""
    attributes = {
...
def test_map_ena_attributes_lat_lon_missing_value_stays_unmapped():
    """An INSDC missing-value marker can't be split into two numbers -- stays local."""
...
def test_map_ena_attributes_leaves_underscore_environmental_context_triad_unmapped():
    """Same ENVO-resolution deferral as the display-name form, for the underscore names."""
    attributes = {
...
def test_map_ena_attributes_both_vocabularies_coexist_when_targets_differ():
    """Display-name and underscore forms are additive when they target distinct fields."""
    attributes = {
    ...
    assert mapped == {
        ...
        # latitude is overwritten by lat_lon's split (later in iteration order): "later wins".
        "geographic location (latitude)": "35.6895",
        "geographic location (longitude)": "139.6917",
        "depth": "10",
    }
