"""Shared field-by-field assertions against the PRJNA48739 fixture data
(`fixtures/`), run by both `test_miint_resolver.py` and
`test_http_resolver.py` so the two `EnaResolver` implementations are held to
the identical contract rather than two independently-drifting test suites."""
