"""Tests for `ena_import.platform_mapping.map_ena_platform`: case-normalized mapping from
ENA's `instrument_platform` to `Platform`, fail-loud outside the closed table."""
...
def test_map_ena_platform_bgiseq_and_dnbseq_agree():
    """BGISEQ is ENA's legacy name for DNBSEQ; both must resolve to the same platform so
    a study mixing the two spellings groups into one sequencing_run."""
