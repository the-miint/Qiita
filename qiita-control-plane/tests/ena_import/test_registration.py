"""DB-bound tests for `ena_import.registration.register_ena_study` against a real
Postgres: study upsert, cross-study biosample de-dup, one sequenced_sample per run,
...
# deliberately-unmapped tags (`host` + the ENVO-typed environmental-context triad).
# ---------------------------------------------------------------------------
...
def _mixs_sample_attributes(sample_accession: str, **overrides: str) -> EnaSampleAttributes:
    attributes = {
        ...
        # Hard value: an INSDC missing-value string on a NUMERIC field -- resolves as a
        # MissingReasonRef via known_missing_reasons, not a MetadataParseError.
        "depth": "not collected",
        # Deliberately unmapped: ENVO-typed fields, retained as local metadata.
        "broad-scale environmental context": "marine biome",
        "local environmental context": "coastal water",
        "environmental medium": "sea water",
        # Deliberately unmapped -- free text, not an NCBI taxon id.
        "host": "Homo sapiens",
    }
...
async def test_harmonized_attributes_land_on_global_fields_and_checklist(reg):
    ...
    # Mapped attrs land on the correct biosample_global_fields.
    ...
    # collection_date is TEXT (rebound from DATE) -- verbatim ISO8601 text.
    ...
    # 'depth' = 'not collected' resolves as a missing-reason marker, not a parse error.
    ...
    # Unmapped tags retained as local TEXT metadata -- 'host' + the env-context triad.
    ...
    assert local_by_display_name == {
        "host": "Homo sapiens",
        # Spelled exactly like their ENVO-typed globals, which is why they are
        # retained locally rather than passed to the import as global metadata.
        "broad-scale environmental context": "marine biome",
        "local environmental context": "coastal water",
        "environmental medium": "sea water",
        # ENA's submitter-supplied sample_alias, as the owner-biosample-id.
        "ena sample id": run.sample_alias,
    }
...
async def test_harmonization_parse_failure_isolated_to_its_run(reg):
    ...
    # 'latitude' is NUMERIC-typed; non-numeric, non-missing-marker text is a parse
    # error, not a missing-required-field gap.
...
async def test_blank_sample_alias_falls_back_to_the_accession(reg):
    ...
    assert result.ena_runs[0].status == EnaRunRegistrationStatus.REGISTERED
    owner_id = await reg["pool"].fetchval(
        "SELECT bm.value_text"
        " FROM qiita.biosample_metadata bm"
        " JOIN qiita.biosample_study_field bsf ON bsf.idx = bm.biosample_study_field_idx"
        " JOIN qiita.biosample b ON b.idx = bm.biosample_idx"
        " WHERE b.ena_sample_accession = $1 AND bsf.display_name = 'ena sample id'",
        sample_accession,
    )
    assert owner_id == sample_accession
...
# The same de-dup under REAL concurrency (asyncio.gather over two register_ena_study
# calls sharing a sample_accession): the biosample get-or-create must still yield ONE
# biosample row + TWO biosample_to_study links when its callers race.
async def test_concurrent_registration_of_shared_biosample_dedupes_to_one_row(reg):
    ...
    # Exactly one run harmonized (created the biosample); the other reused it --
...
async def test_paired_and_single_layout_runs_each_get_one_sequenced_sample(reg):
    ...
    # Reserved-range invariant (>= 25000) holds ONLY for study and prep_sample --
    # biosample / sequenced_sample / sequencing_run legitimately mint low.
...
# ---------------------------------------------------------------------------
# Mixed-platform study groups into multiple sequencing_run / sequenced_pool rows,
# and protocol mapping is persisted.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# created_pools -- surfaces the (platform, sequenced_pool_idx, sequencing_run_idx)
# triples the batch driver needs to build one download ticket per pool.
# ---------------------------------------------------------------------------
...
async def test_partial_failure_run_leaves_no_orphan_rows_and_rerun_completes(reg):
    ...
    bad_run = _run(
        ...
        sample_accession=bad_sample_accession,
        study_accession=study_accession,
        # ChIP-Seq has no curated-protocol mapping -- this run fails in its own txn.
    ...
    # The per-run txn rolled back entirely -- confirm no biosample-to-study link survived.
    ...
    # Re-run: the ok run is skipped; the previously-bad run, re-submitted with a mappable
    # strategy, completes -- the partial failure left the identity space clean.
    fixed_run = _run(
        run_accession=bad_run_accession,
...
# ---------------------------------------------------------------------------
# Unmappable-platform failure isolation -- a bad `instrument_platform` fails only
# that run, never the whole study import.
# ---------------------------------------------------------------------------
...
async def test_unmappable_platform_run_isolated_others_registered(reg):
    ...
    bad_run = _run(
        ...
        sample_accession=bad_sample_accession,
        study_accession=study_accession,
        # CAPILLARY has no qiita.platform counterpart.
        instrument_platform="CAPILLARY",
    )
    ...
    # Exactly one sequencing_run / sequenced_pool -- only the good run's ILLUMINA.
    ...
    # No orphan rows for the platform-failed run -- it fails before any per-run write,
    # unlike the protocol-mapping case above.
