...
with a mocked httpx transport + a fake Flight client, both from
`tests/cli/conftest.py`. The full integration path (real CP + real DP
subprocess + DuckLake) lives in `tests/integration/test_e2e_reference.py`; this
file exercises only the CLI orchestration: the call sequence, error
propagation, and the Arrow-conversion helpers.
"""
