...
-- =============================================================================
-- METADATA CHECKLIST FIELDS (dual-keyed; biosample-side and sequenced-sample-side)
-- =============================================================================
...
CREATE TABLE qiita.metadata_checklist_field (
    ...
    CONSTRAINT metadata_checklist_field_exactly_one_target CHECK (
        (biosample_global_field_idx IS NOT NULL AND sequenced_sample_global_field_idx IS NULL)
        OR
        (biosample_global_field_idx IS NULL AND sequenced_sample_global_field_idx IS NOT NULL)
    )
);

COMMENT ON TABLE qiita.metadata_checklist_field IS
    'Required fields for a metadata checklist, dual-keyed to either a global '
    'biosample field or a global sequenced-sample field. A checklist like MIxS '
    'describes requirements at both layers (biosample collection metadata and '
    'sequenced-sample sequencing/library metadata), so the table needs to '
...
-- At most one row per (metadata_checklist, global sequenced-sample field) pair.
CREATE UNIQUE INDEX metadata_checklist_field_unique_sequenced
    ON qiita.metadata_checklist_field (metadata_checklist_idx, sequenced_sample_global_field_idx)
...
CREATE INDEX metadata_checklist_field_sequenced_idx
    ON qiita.metadata_checklist_field (sequenced_sample_global_field_idx)
    WHERE sequenced_sample_global_field_idx IS NOT NULL;
