...
COMMENT ON TABLE qiita.prep_protocol IS
    ...
    'A prep protocol classifies a prep sample by the laboratory procedure '
    'used to prepare it. Via the prep_protocol_field join table (defined below), '
    'it declares which prep_sample fields are allowed (and which are '
    'required) for prep samples following that protocol; writes are '
...
COMMENT ON COLUMN qiita.prep_protocol.retired IS
    'When true, this prep protocol is soft-hidden. Prep samples already '
    'referencing it continue to function; new prep samples cannot be '
    ...
    'prep_sample.prep_protocol_idx uses ON DELETE RESTRICT.';
...
-- =============================================================================
-- PREP SAMPLE FIELDS (the field registry, split global vs. study-scoped)
-- =============================================================================

CREATE TABLE qiita.prep_sample_global_field (
    idx               BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
    ...
    CONSTRAINT prep_sample_global_field_internal_name_unique UNIQUE (internal_name),
    CONSTRAINT prep_sample_global_field_internal_name_format
    ...
    CONSTRAINT prep_sample_global_field_terminology_data_type_consistent
...
CREATE TABLE qiita.prep_sample_study_field (
    ...
    CONSTRAINT prep_sample_study_field_display_name_unique
    ...
    CONSTRAINT prep_sample_study_field_inheritance_consistent
        CHECK (
...
CREATE INDEX prep_sample_study_field_study_idx
    ON qiita.prep_sample_study_field (study_idx);
CREATE INDEX prep_sample_study_field_global_link_idx
    ON qiita.prep_sample_study_field (prep_sample_global_field_idx)
    WHERE prep_sample_global_field_idx IS NOT NULL;


-- =============================================================================
-- PREP PROTOCOL FIELDS (prep_protocol x prep_sample_global_field association)
-- =============================================================================
...
CREATE TABLE qiita.prep_protocol_field (
    ...
    prep_sample_global_field_idx  BIGINT NOT NULL REFERENCES qiita.prep_sample_global_field(idx) ON DELETE RESTRICT,
    required                           BOOLEAN NOT NULL DEFAULT false,
    created_by_idx                     BIGINT NOT NULL REFERENCES qiita.principal(idx) ON DELETE RESTRICT,
    created_at                         TIMESTAMPTZ NOT NULL DEFAULT now(),

    PRIMARY KEY (prep_protocol_idx, prep_sample_global_field_idx)
);

COMMENT ON TABLE qiita.prep_protocol_field IS
    'Many-to-many between prep protocols and prep-sample global fields. '
...
-- Initial set of prep protocols recognized by the system. created_by_idx=1
-- references the system principal seeded earlier; descriptions are left NULL
-- and may be filled in by admins later via the registry surface.

INSERT INTO qiita.prep_protocol (name, created_by_idx) VALUES
    ('short_read_metagenomics',    1),
    ('short_read_transcriptomics', 1),
    ('long_read_metagenomics',     1),
    ('short_read_amplicon',        1),
    ('long_read_amplicon',         1);


-- =============================================================================
-- PREP-SAMPLE SUBMISSION GLOBAL FIELDS (seed)
-- =============================================================================

-- Globally-known prep-sample fields required for ENA Experiment
...
DROP TABLE IF EXISTS qiita.prep_sample_study_field;
DROP TABLE IF EXISTS qiita.prep_sample_global_field;
