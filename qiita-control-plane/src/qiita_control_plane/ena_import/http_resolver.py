"""`HttpEnaResolver` — the experimental plain-HTTP fallback (off by default).

Implements the `EnaResolver` contract by talking to ENA's public APIs over `httpx`
instead of DuckDB + miint: the Portal API (`/search`, TSV) for study header and runs,
and the Browser API (`/xml/...`) for per-sample attributes. URL/query shape mirrors
`duckdb-miint`'s `ENAParser::BuildSearchURL` / `BuildXMLURL` so the two resolvers agree
...
def _parse_tsv(text: str) -> tuple[list[str], list[list[str]]]:
    """Parse a `format=tsv` Portal API response into (columns, rows). ENA always
    returns a header line even for zero data rows."""
...
def _parse_sample_attributes_xml(xml_text: str) -> list[list[str]]:
    """Parse a Browser API `/xml/<accession,...>` response into (sample_accession,
    tag, value) rows — the same narrow shape `read_ena_attributes` returns."""
...
class HttpEnaResolver(EnaResolver):
    """Experimental — talks to ENA's public HTTP APIs directly, no miint / DuckDB
    dependency. Off by default; see `ena_import.get_resolver`."""
    ...
    def resolve_sample_attributes(self, accession: str) -> list[EnaSampleAttributes]:
        accession = validate_study_accession(accession)
        # Genuine existence check (mirrors resolve_runs): zero samples for a
        # well-formed study means nothing resolved, so raise.
        ...
        if not rows:
            # Unlike the existence check above, zero <SAMPLE_ATTRIBUTE> rows across a
            # known-real sample set is a legitimate "no attributes" result -- a real
            # ENA/DDBJ sample can carry none. Return [] rather than raise;
