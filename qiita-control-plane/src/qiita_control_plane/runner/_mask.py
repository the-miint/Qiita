...
def _build_mask_params(
    *,
    ...
    (the host refs the filter applies + the adapter-set identity + thresholds), so
    two callers with the same effective config collapse to one mask even if
    descriptive metadata differs. `adapter_set_hash` is passed in already computed
    (`reference_sequence_set_hash`, over the adapter reference's sequence hashes)
    rather than as a reference idx, so a caller can also pass the legacy
    byte-derived value to build the fallback lookup key — see
