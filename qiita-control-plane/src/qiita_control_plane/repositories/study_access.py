...
class CallerStudyAccessRow(NamedTuple):
    """Owner and caller-tier for one (caller, study) pair.
    ...
    interpretation is policy-layer, not data-layer).
    """
...
async def fetch_caller_study_access(
    ...
    *,
    ...
    row = await conn.fetchrow(
        "SELECT s.owner_idx, sa.access_tier"
        " FROM qiita.study s"
    ...
    return CallerStudyAccessRow(owner_idx=row["owner_idx"], access_tier=access_tier)
