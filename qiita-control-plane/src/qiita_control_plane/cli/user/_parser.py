...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_pool_completion = sub.add_parser(
        ...
        description=(
            ...
            " in-flight / no-data / failed / cancelled / not-submitted), with a"
            " `complete` flag set when every sample reached a usable or"
            ...
            " including samples a partial fan-out never submitted"
            " (`samples_not_submitted`) and runs withdrawn after the fact"
            " (`samples_invalidated`), which are counted but are NOT usable."
            " Whether a sample is masked is read from the same gate the masked-read"
            " pull reads, so this summary and that pull agree. Compute-on-read — it"
            " never drifts when a sample is re-processed, withdrawn, or deleted."
        ),
    )
