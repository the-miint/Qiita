...
- **The QC adapter set refuses a duplicated chunk position instead of concatenating it
  ...
  `chunk_index` 0 — an exact reverse-complement pair the strand-folding feature hash mints
  into one feature — so the join returned 66 bp for a 33 bp adapter. It now raises naming
  every repeated `(feature_idx, chunk_index)`, which `_resolve_qc_adapters` turns into a
  SUBMISSION `BAD_INPUT` with no partial `adapters.parquet` left behind.
  `DEPLOY_CHECKLIST.md` bucket 6 carries the one-off delete that repairs the row and picks
  the orientation `fastp_truseq_adapters.fna` declares.
